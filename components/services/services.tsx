  ...
  return isDesktop ? (
    <section className='hidden lg:flex flex-col justify-between gap-8 col-span-7 bg-grey-bg px-6 pt-10 pb-6 rounded-2xl'>
      <div className='flex flex-col gap-5 mt-2'>
        <p className='text-2xl 4xl:text-3xl text-foreground'>What we do:</p>
...
const ServiceCard = ({
  ...
  return (
    <article
      className={cn(
        'absolute inset-0 flex flex-col gap-16 h-full justify-between rounded-2xl p-6 transition-all ease-out duration-1000 opacity-0',
    ...
    >
      <div
        className={cn(
          'transition-all ease-out duration-1000 opacity-0 ',
          isActive && 'opacity-100'
        )}
      >
      ...
      <div>
        <p
          className={cn(
            'text-white text-lg 4xl:text-2xl transition-all ease-out duration-1000 opacity-0',
            isActive && 'opacity-100'
          )}
        >
          {service.description}
